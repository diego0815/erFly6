...
{
	lcd_putsAttIdx( x, y, Str_TXeq, ( g_model.protocol == PROT_AFHDS2A ), 0 ) ;
}
...
void menuProcTelemetry(uint8_t event)
{
	...
	static MState2 mstate2;
#ifdef V2
	mstate2.check_columns(event, 22-1-1+6-1) ;
#else
#ifdef MAH_LIMIT			 
  	...
  	k=i+t_pgOfs;
	    ...
	    if (j == 0)
    		...
    		putsChn(0,y,k+1,0);
				...
				value = checkIndexed( y, PSTR(FWx5"\001""\006SafetySticky"), value, active ) ;
				sd->mode = value ;
			}
      else if (j == 1)
			{
          ...
          if(active)
            ...
            CHECK_INCDEC_MODELSWITCH( sd->opt.ss.swtch, -MaxSwitchIndex, max ) ;
				...
				else
				{
         			...
         			lcd_putsAttIdx(20*FW, y, Str_Sounds, sd->opt.ss.val,attr);
						}
...
//TODO Kotello telemetry
/*****************************************************************************/

		displayTimer(17 * FW, FH * 4, 1, INVERS);

#endif // SMALL_DBL
		putsTmrMode(12 * FW , FH * 4, INVERS, 0x80);

		lcd_puts_P(2 + 1 * FW, 4 * FH, PSTR("IntV="));
		lcd_puts_P(2 + 1 * FW, 5 * FH, PSTR("ExtV="));
		lcd_puts_P(2 + 1 * FW, 6 * FH, PSTR("Err ="));
		lcd_puts_P(   11 * FW, 5 * FH, PSTR("RPM ="));
		lcd_puts_P(   11 * FW, 6 * FH, PSTR("Temp="));

		if (telem_status & (1 << RX_IntV))
			putsVolts(9 * FW, 4 * FH, (telem_AFHDS2A[RX_IntV] / 10), 0);
		else
			lcd_putsAtt(7 * FW, 4 * FH, "---", 0);

		if (telem_status & (1 << RX_ExtV))
			putsVolts(9 * FW, 5 * FH, (telem_AFHDS2A[RX_ExtV] / 10), 0);
		else
			lcd_putsAtt(7 * FW, 5 * FH, "---", 0);

		if (!(telem_status & (1 << RX_Err)))
			telem_AFHDS2A[RX_Err] = 100;
		lcd_outdezAtt(9 * FW, 6 * FH, telem_AFHDS2A[RX_Err], 0);
		lcd_putcAtt(Lcd_lastPos, 6 * FH, '%', 0);

		if (telem_status & (1 << RX_RPM))
			lcd_outdezAtt(19 * FW, 5 * FH, telem_AFHDS2A[RX_RPM], 0);
		else
			lcd_putsAtt(17 * FW, 5 * FH, "---", 0);

		if (telem_status & (1 << RX_Temp))
			lcd_outdezAtt(19 * FW, 6 * FH, telem_AFHDS2A[RX_Temp], 0);
		else
			lcd_putsAtt(17 * FW, 6 * FH, "---", 0);
...
//	uint8_t b ;
	...
	subN++;
	y += FH;
	lcd_puts_Pleft(y,"Sub Protocol\037Servo Freq Hz\037Freq Offset");
    ...
    lcd_outdezAtt(PARAM_OFS,y,g_model.FreqOffset,attr);
}
/*********************************************************************************/
